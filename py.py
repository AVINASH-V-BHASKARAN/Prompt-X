...
if "--simulate" in sys.argv or "--benchmark" in sys.argv or "simulate" in sys.argv or "benchmark" in sys.argv or not API_KEY or not genai:
    SIMULATION_MODE = True
    if not API_KEY:
        print("[INFO] GEMINI_API_KEY not detected. Running in SIMULATION MODE with Adrian Vale simulation engine.")
    else:
    ...
    try:
        ...
        print(f"[WARN] Failed to initialize Gemini Client ({e}). Falling back to SIMULATION MODE.")
        SIMULATION_MODE = True
...
EVIDENCE_DEFINITIONS = {
    ...
    "daniel_files": {
        ...
        "keywords": ["echo", "projectecho", "wipe", "audit", "developer signature", "signature", "ip address", "2128", "928", "tampering", "altered", "manipulated", "fake records", "falsified", "audit records", "files"]
    },
    "physical_clue": {
...
def analyze_question(question: str, state: GameState) -> dict:
    ...
    def kw_match(pattern: str, text: str) -> bool:
    ...
    if any(kw_match(term, norm_q) for term in ["chemical", "compound", "slip", "powder", "material", "rubber", "weapon"]):
    ...
    if any(kw_match(k, norm_q) for k in ["2115", "915", "left", "walked out", "departure", "present after", "still there", "stayed"]):
        contradictions.append("timeline")
        facts_referenced.append("adrian_present_after_21_15")

    # 2. Location: archive / basement / corridor
    if any(kw_match(k, norm_q) for k in ["archive", "subbasement", "basement", "corridor", "hallway", "inside"]):
        contradictions.append("location")
        facts_referenced.append("archive_access_subbasement")

    # 3. Contact: phone, 17:00, meeting Daniel
    if any(kw_match(k, norm_q) for k in ["contact", "spoke", "talk", "daniel", "1700", "500", "5 pm", "5pm"]):
        contradictions.append("victim_contact")

    # 4. Motive: altered files, records, audit, framing, reason to silence
    if any(kw_match(k, norm_q) for k in ["motive", "altered", "manipulated", "investigation", "framing", "destroy", "tamper", "cover up", "silent"]):
...
PRESSURE_TO_EVIDENCE = {
    "card": "access_card",
    "access_card": "access_card",
    "location": "cctv",
    "cctv": "cctv",
    "contact": "phone_records",
    "phone_records": "phone_records",
    "motive": "daniel_files",
    "daniel_files": "daniel_files",
    "physical_clue": "physical_clue",
    "physical": "physical_clue"
}


def select_defence(evidence_id: str, state: GameState) -> str:
    mapped_id = PRESSURE_TO_EVIDENCE.get(evidence_id.lower(), evidence_id.lower())
    candidates = AVAILABLE_DEFENCES.get(mapped_id, ["general_denial"])
...
# ============================================================
# DETERMINISTIC SIMULATION RESPONSE GENERATOR
# ============================================================

def generate_simulated_response(question: str, state: GameState, strategy: str, pressure_point: str, chosen_defence: str) -> str:
    norm_q = normalize_text(question)
    stress = state.stress

    # 1. CARD / TIMELINE
    if any(k in norm_q for k in ["0890", "keycard", "card", "badge", "2139", "939", "fire door"]):
        if chosen_defence == "card_cloned_or_borrowed":
            if stress < 40:
                return "Keycard records don't prove who physically held the card. Anyone in the department could clone an RFID badge or pick one up off a desk. That doesn't place me in the sub-basement."
            elif stress < 80:
                return "I've misplaced my badge multiple times. Someone picking it up off my workstation after I left doesn't prove I was holding it at 9:39."
            else:
                return "I told you, RFID cards aren't fingerprint scanners! Someone could easily have taken my card while I was out at the coffee shop to set me up!"
        elif chosen_defence == "card_left_on_desk":
            return "I took off my lanyard and left it sitting right by my keyboard before I walked out at 21:15. Check with anyone on the floor—it was sitting right in the open."
        else:  # system_timestamp_glitch
            return "The access logs desynchronize all the time. Facilities has had open trouble tickets about sub-basement reader lag for months."

    # 2. CCTV / LOCATION
    if any(k in norm_q for k in ["cctv", "camera", "reflection", "coat", "2137", "937", "surveillance", "footage"]):
        if chosen_defence == "reflection_is_distorted":
            if stress < 40:
                return "A blurry reflection in a hallway display case at 9:37? That's barely an outline, let alone an identification. It could be anyone walking down that hall."
            elif stress < 80:
                return "You're building a whole timeline on a curved glass reflection? The lighting down there distorts everything. It doesn't prove it was me."
            else:
                return "You can't even see a face on that feed! Half the project leads wear charcoal wool coats in the winter—it proves nothing!"
        elif chosen_defence == "corridor_does_not_mean_archive":
            return "Even if someone was in Corridor B, that hallway leads to five different supply closets and the network hub, not just the archive."
        else:  # coat_is_not_unique
            return "It's an off-the-rack dark wool overcoat from a mass retailer. Three other project leads in our wing own the exact same coat."

    # 3. PHONE RECORDS / CONTACT
    if any(k in norm_q for k in ["phone", "call", "14 second", "14second", "14s", "cellular", "antenna", "2132", "932"]):
        if chosen_defence == "automatic_system_ping":
            if stress < 40:
                return "A 14-second link isn't a conversation. That's the automated PBX polling cycle between workstation extensions."
            elif stress < 80:
                return "A 14-second network handoff happens whenever extensions resync. Daniel wasn't even at his desk when I left."
            else:
                return "I didn't speak with him! I might have dialed his desk extension to ask about tomorrow's sprint... wait, no, the line just rang out!"
        elif chosen_defence == "call_does_not_prove_conversation":
            return "Fourteen seconds is barely three rings and a voicemail beep. It doesn't prove two people actually spoke a single word."
        else:  # network_routing_artifact
            return "Our internal VoIP routing queue creates ghost logs when a packet drops. Check the network diagnostics before pointing fingers."

    # 4. DANIEL'S FILES / MOTIVE
    if any(k in norm_q for k in ["echo", "projectecho", "wipe", "audit", "signature", "ip address", "2128", "928", "tampering", "falsified"]):
        if chosen_defence == "ip_address_was_spoofed":
            if stress < 40:
                return "An internal IP address can be spoofed in thirty seconds by any developer on the local subnet. It proves nothing about who sat at the terminal."
            elif stress < 80:
                return "PROJECT_ECHO was an open directory. Anyone with network access could route a purge command through my terminal IP."
            else:
                return "Daniel didn't understand what he was looking at! He was going to wreck months of company work over routine database cleanup!"
        elif chosen_defence == "old_signature_reused_in_codebase":
            return "My developer signature is in dozens of open repository templates. Anyone building a script could have copied that header."
        elif chosen_defence == "routine_maintenance_misinterpreted":
            return "Daniel was paranoid. That was a scheduled maintenance cleanup for legacy data, not a malicious wipe."
        else:  # daniel_looked_into_irrelevant_files
            return "Daniel was chasing shadows. The files he flagged had nothing to do with any wrongdoing."

    # 5. PHYSICAL EVIDENCE / GLOVE
    if any(k in norm_q for k in ["glove", "nitrile", "paperweight", "weapon", "chemical", "fragment"]):
        if chosen_defence == "gloves_are_standard_issue":
            if stress < 40:
                return "Blue nitrile gloves are standard issue across the entire lab. Boxes of them are in every utility cabinet."
            elif stress < 80:
                return "Finding a speck of nitrile rubber on an archive desk means nothing. Everyone wears them when handling hardware."
            else:
                return "My desk had open boxes because people borrow them constantly! That doesn't put the weapon in my hand!"
        elif chosen_defence == "anyone_could_take_gloves_from_desk":
            return "People walk by my desk and grab gloves all the time for server maintenance. Why am I responsible for who took them?"
        else:  # contamination_in_shared_lab
            return "Forensics picking up micro-fragments in a shared archive? That's ambient trace material from months of shared use."

    # 6. MULTI-EVIDENCE / GENERAL / BREAKING
    if stress >= 96:
        return "I... I can't keep explaining away every single timestamp... you don't understand what happened down there with Daniel threatening everything!"
    elif stress >= 80:
        return "You're throwing all these pieces at me like they form a neat picture, but you're ignoring everyone else who had access to those files!"
    elif stress >= 60:
        return "How many times do I have to repeat myself? I left at 21:15, and taking routine workplace logs out of context won't change that."
    elif stress >= 40:
        return "I'm answering your questions, but you're jumping to conclusions without real proof. You need to look at other people on the night shift."
    else:
        return "I already told the detectives: I signed out of the office at 21:15 and walked over to 'The Grind & Log' for a coffee until 22:00. Check the register."
...
async def ask_adrian_with_validator(question: str, state: GameState, pressure_point: str) -> dict:
    ...
    # If in simulation mode or Gemini client is not configured, use the deterministic simulation generator
    if SIMULATION_MODE or client is None:
        answer = generate_simulated_response(question, state, strategy, pressure_point, chosen_defence)
        elapsed = max(0.05, round(time.perf_counter() - start_time, 2))
        if answer:
            state.recent_responses.append(answer)
        return {
            "answer": answer,
            "time": elapsed,
            "success": True,
...
if __name__ == "__main__":
    ...
    run_sim = any(a in cli_args for a in ["--simulate", "-s", "simulate", "--benchmark", "-b", "benchmark", "test"])
    
    if run_sim or not sys.stdin.isatty():
